...
本仓库是我围绕 Kronos 金融时间序列基础模型开展的个人探索版本。我的工作重点不在于重新提出 Kronos 模型本身，而是尝试将其接入到更完整的量化研究流程中：包括 A 股/港股 K 线数据获取与整理、基于预训练模型的价格序列预测、预测结果可视化、历史回测、策略验证，以及针对本地 CSV 金融数据的微调流程探索。

在这个项目中，我主要关注三个问题：第一，Kronos 这类金融基础模型如何适配真实市场数据；第二，模型预测结果如何转化为可检验的交易信号；第三，预测、回测和策略评估之间如何形成闭环。仓库中保留了我早期的实验脚本、回测尝试和可视化结果，作为后续继续研究“深度学习/基础模型 + 量化交易”方向的探索基础。
